...
app.get("/PublicBanks",function(req,res){
  organisations.find({type:"Public Bank"},function(err,foundBanks){
    if(err)
    {
      console.log(err);
    } else{
      res.render("PublicBanks",{publicbank:foundBanks});
    }
  });
});

app.get("/PrivateBanks",function(req,res){
  organisations.find({type:"Private Bank"},function(err,foundBanks){
    if(err)
    {
      console.log(err);
    } else{
  res.render("PrivateBanks",{privatebank:foundBanks});
    }
  });
});

app.get("/NBFCs",function(req,res){
  organisations.find({type:"NBFC"},function(err,foundBanks){
    if(err)
    {
      console.log(err);
    } else{
  res.render("NBFC",{nbfc:foundBanks});
    }
  });
});

app.get("/SFCs",function(req,res){
  organisations.find({type:"SFC"},function(err,foundBanks){
    if(err)
    {
      console.log(err);
    } else{
  res.render("SFC",{sfc:foundBanks});
    }
  });
});
////
